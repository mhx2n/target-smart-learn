            ...
            <div key={banner.id} className="container py-1.5 flex items-center gap-3 justify-center relative">
              {banner.image && (
                <img
                  src={banner.image}
                  alt={banner.caption}
                  className="w-7 h-7 sm:w-9 sm:h-9 rounded-lg object-cover shrink-0"
                />
              )}
              <div className="flex flex-col sm:flex-row items-center gap-0.5 sm:gap-3 text-center min-w-0">
                ...
                <span className="text-[10px] sm:text-xs font-mono font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full whitespace-nowrap">
