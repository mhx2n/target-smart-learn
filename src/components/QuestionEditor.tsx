      ...
      <div className="min-h-full flex flex-col">
        <div className="sticky top-0 z-10 bg-card border-b border-border px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={saveAll} disabled={upsertExam.isPending} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 transition-all">
              <Save size={16} /> {upsertExam.isPending ? "সেভ হচ্ছে..." : "সংরক্ষণ"}
            </button>
            <button onClick={onClose} className="p-2.5 rounded-xl hover:bg-muted transition-colors"><X size={22} /></button>
          </div>
        </div>
                    ...
                    <div>
                      ...
                      {q.questionImage ? (
                        <div className="relative inline-block">
                          ...
                          <button onClick={() => removeQuestionImage(q.id)} className="absolute top-2 right-2 p-2 rounded-full bg-destructive text-destructive-foreground shadow-lg"><X size={16} /></button>
                        </div>
                      ) : (
                        <label className="flex items-center justify-center gap-3 w-full py-8 rounded-xl border-2 border-dashed border-border text-sm text-muted-foreground cursor-pointer hover:border-primary/50 hover:text-primary hover:bg-primary/5 transition-all">
                          <ImagePlus size={24} /><span>ছবি যোগ করতে ট্যাপ করুন</span>
                      ...
                      {q.options.map((opt, oi) => (
                        <div key={oi} className={`p-4 rounded-xl border-2 ${opt === q.answer ? "border-success/60 bg-success/5" : "border-border"}`}>
                          <div className="flex items-center gap-3 mb-3">
                            <span className="w-9 h-9 rounded-full bg-muted text-sm flex items-center justify-center font-bold flex-shrink-0">{String.fromCharCode(65 + oi)}</span>
                          ...
                          <div className="flex items-center gap-3 ml-12">
                            <button onClick={() => updateQ(q.id, { answer: opt })}
                            ...
                            {q.optionImages?.[oi] ? (
                              <div className="relative inline-block">
                                ...
                                <button onClick={() => removeOptionImage(q.id, oi)} className="absolute top-1 right-1 p-1.5 rounded-full bg-destructive text-destructive-foreground shadow-lg"><X size={12} /></button>
                              </div>
                            ) : (
                              <label className="flex items-center gap-2 px-4 py-2 rounded-lg border-2 border-dashed border-border text-xs text-muted-foreground cursor-pointer hover:border-primary/50 hover:text-primary transition-all">
                                <ImagePlus size={16} /><span>ছবি যোগ</span>
